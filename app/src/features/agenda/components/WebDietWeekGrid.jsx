...
function isFullDayBlocked(blockedDates, ymd) {
  return blockedDates.some(item => (typeof item === 'string' ? item : item.date) === ymd && typeof item === 'string');
}

function getPeriodBlocks(blockedDates, ymd) {
  return blockedDates.filter(item => typeof item !== 'string' && item.start && item.end && (item.recurrence === 'daily' || item.date === ymd));
}

function getBlockPosition(block) {
  const [startHour, startMinute] = block.start.split(':').map(Number);
  const [endHour, endMinute] = block.end.split(':').map(Number);
  const start = Math.max(START_HOUR * 60, startHour * 60 + startMinute);
  const end = Math.min((END_HOUR + 1) * 60, endHour * 60 + endMinute);
  return { top: (start - START_HOUR * 60), height: Math.max(0, end - start) };
}
...
export default function WebDietWeekGrid({
  ...
  const weekDays = useMemo(() => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const isBlocked = isFullDayBlocked(blockedDates, ymd);
  ...
  const monthMatrix = useMemo(() => {
    ...
    while (curr <= endDate) {
      ...
      days.push({
        ...
        isBlocked: isFullDayBlocked(blockedDates, ymd)
      });
              ...
              {weekDays.map((day, dayIndex) => {
                ...
                return (
                  ...
                  >
                    ...
                    {getPeriodBlocks(blockedDates, day.ymd).map((block, index) => {
                      const position = getBlockPosition(block);
                      return position.height > 0 ? (
                        <div key={`blocked-period-${index}`} className="absolute left-0 right-0 bg-rose-950/65 border-y border-rose-400/60 z-20 pointer-events-auto flex items-center justify-center" style={{ top: `${position.top}px`, height: `${position.height}px` }} title={`Bloqueado: ${block.start}–${block.end}`}>
                          <span className="text-[10px] font-bold uppercase text-rose-100 bg-rose-900/70 px-2 py-1 rounded-lg">Bloqueado {block.start}–{block.end}</span>
                        </div>
                      ) : null;
                    })}
