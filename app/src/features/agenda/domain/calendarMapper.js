export const colorMapper = {
  ...
  getStatusKey: (event) => {
    if (colorMapper.statuses[event.statusKey]) return event.statusKey;
    const text = ((event.summary || '') + ' ' + (event.description || '')).toLowerCase();
    if (/desmarcado|cancelado|❌/.test(text)) return 'desmarcado';
    if (/não confirmado|nao confirmado|a confirmar|a_confirmar/.test(text)) return 'a_confirmar';
    if (/confirmado|✅/.test(text)) return 'confirmado';
    return 'a_confirmar';
  },
  getStatusByTitleOrDescription: event => colorMapper.statuses[colorMapper.getStatusKey(event)],
