...
export default function Anamnese({ activeModel }) {
  ...
  const callGemini = async (prompt, isJson = false) => {
    ...
    if (!geminiApiKey) {
      showNotification("Chave API do Gemini não configurada no ambiente (.env).", "error");
    ...
    // Cascata exclusivamente Gemini (DeepSeek removido)
    const geminiChain = [
      "gemini-3.8-flash",
      "gemini-2.5-flash",
    ...
    const modelsToTry = [initialModel, ...geminiChain.filter(m => m !== initialModel)];
    ...
    showNotification("Todos os modelos Gemini falharam. Verifique sua conexão ou cota.", "error");
